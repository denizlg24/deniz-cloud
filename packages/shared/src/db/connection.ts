...
export function createDb(connectionString: string, options?: PostgresClientOptions) {
...
export function createRawClient(
  ...
  return postgres(connectionString, postgresOptions({ ...options, max: options?.max ?? 1 }));
}
