...
import { escapeXml, S3Error, s3ErrorResponse, xmlResponse } from "../s3/errors";
import {
...
import { parseCompletedParts, parseDeleteObjects } from "../s3/xml";

interface S3RouteConfig extends S3AuthConfig, S3StoreConfig {
  enabled: boolean;
}
...
function decodeTarget(url: URL): S3Target {
  ...
  try {
    if (separator < 0) return { bucket: decodeURIComponent(relative) };
    ...
    throw new S3Error("InvalidURI", "Could not parse the specified URI.", 400, url.pathname);
  }
}
...
function parseInteger(value: string | null, name: string, fallback?: number): number | undefined {
  ...
  if (!/^\d+$/.test(value)) {
    throw new S3Error("InvalidArgument", `${name} must be a non-negative integer.`, 400);
  }
...
async function readVerifiedBody(request: Request): Promise<string> {
  ...
  if (declaredLength > MAX_CONTROL_BODY_BYTES) {
    throw new S3Error("EntityTooLarge", "The request body exceeds the allowed size.", 413);
  }
  ...
  assertPayloadHash(request.headers.get("x-amz-content-sha256"), actualHash);
...
function listBucketsResponse(
  ...
  const bucketXml = buckets
    ...
    .join("");
  return xmlResponse(
    `<ListAllMyBucketsResult xmlns="${S3_XML_NAMESPACE}"><Owner><ID>${escapeXml(config.accessKeyId)}</ID><DisplayName>deniz-cloud</DisplayName></Owner><Buckets>${bucketXml}</Buckets></ListAllMyBucketsResult>`,
  );
}
...
async function listObjectsResponse(
  ...
  const maxKeys = parseInteger(url.searchParams.get("max-keys"), "max-keys", 1000);
  ...
  const commonPrefixes = result.commonPrefixes
    .map(
      (commonPrefix) =>
        `<CommonPrefixes><Prefix>${encodeS3Key(commonPrefix, encodingType)}</Prefix></CommonPrefixes>`,
    )
    .join("");
  ...
  const delimiterXml = delimiter
    ...
    : "";
  const encodingXml = encodingType ? `<EncodingType>${encodingType}</EncodingType>` : "";
...
function objectHeaders(metadata: Awaited<ReturnType<typeof getObjectMetadata>>): Headers {
...
function applyResponseOverrides(headers: Headers, url: URL): void {
  ...
  for (const [parameter, header] of Object.entries(overrides)) {
    ...
    if (value && [...value].every((character) => character.charCodeAt(0) <= 0x7f)) {
...
function checkConditions(
  ...
  if (ifUnmodifiedSince && new Date(lastModified) > new Date(ifUnmodifiedSince)) {
    return new Response(null, { status: 412 });
  }
  return undefined;
}
...
async function getObjectResponse(
  ...
  const conditional = checkConditions(request, metadata.etag, metadata.lastModified);
  ...
  if (!match || (!match[1] && !match[2])) {
    throw new S3Error("InvalidRange", "The requested range is not satisfiable.", 416, key);
  }
  let start: number;
  let end: number;
  if (!match[1]) {
...
function parseCopySource(value: string): { bucket: string; key: string } {
  try {
    ...
    return { bucket: decoded.slice(0, separator), key: decoded.slice(separator + 1) };
  } catch {
...
async function deleteObjectsResponse(
  ...
  const { keys, quiet } = parseDeleteObjects(body);
  if (keys.length > 1000) {
    throw new S3Error("MalformedXML", "A maximum of 1000 keys can be deleted per request.", 400);
  }
  await Promise.all(keys.map((key) => deleteObject(config, bucket, key)));
  const deleted = quiet
    ? ""
    : keys.map((key) => `<Deleted><Key>${escapeXml(key)}</Key></Deleted>`).join("");
  return xmlResponse(`<DeleteResult xmlns="${S3_XML_NAMESPACE}">${deleted}</DeleteResult>`);
}

async function multipartUploadsResponse(config: S3RouteConfig, bucket: string): Promise<Response> {
  const uploads = await listMultipartUploads(config, bucket);
  const items = uploads
    .map(
      (upload) =>
        `<Upload><Key>${escapeXml(upload.key)}</Key><UploadId>${escapeXml(upload.uploadId)}</UploadId><Initiator><ID>${escapeXml(config.accessKeyId)}</ID><DisplayName>deniz-cloud</DisplayName></Initiator><Owner><ID>${escapeXml(config.accessKeyId)}</ID><DisplayName>deniz-cloud</DisplayName></Owner><StorageClass>STANDARD</StorageClass><Initiated>${escapeXml(upload.initiated)}</Initiated></Upload>`,
    )
    .join("");
  return xmlResponse(
...
async function dispatchS3(config: S3RouteConfig, request: Request): Promise<Response> {
  const url = new URL(request.url);
  const { bucket, key } = decodeTarget(url);
  const method = request.method.toUpperCase();

  if (!bucket) {
    if (method === "GET") return listBucketsResponse(config, await listBuckets(config));
    throw new S3Error(
      "MethodNotAllowed",
  ...
  if (!key) {
    if (method === "PUT") {
      ...
      return new Response(null, { status: 200, headers: { Location: `/v2/${bucket}` } });
    }
    if (method === "HEAD") {
      await headBucket(config, bucket);
      return new Response(null, { headers: { "x-amz-bucket-region": config.region } });
    }
    if (method === "DELETE") {
    ...
    if (method === "GET" && url.searchParams.has("uploads")) {
      return multipartUploadsResponse(config, bucket);
    }
    if (method === "GET") return listObjectsResponse(config, bucket, url);
    throw new S3Error(
      "MethodNotAllowed",
      "The specified method is not allowed against this resource.",
      405,
  ...
  if (method === "PUT" && uploadId && url.searchParams.has("partNumber")) {
    const partNumber = parseInteger(url.searchParams.get("partNumber"), "partNumber");
    const part = await uploadPart(config, bucket, key, uploadId, partNumber ?? 0, request);
  ...
  if (method === "PUT") {
    ...
    if (copySource) {
      ...
      const metadata = await copyObject(config, source.bucket, source.key, bucket, key, request);
      return xmlResponse(
    ...
    return new Response(null, { headers: { ETag: `"${metadata.etag}"` } });
  }
...
export async function initializeS3(config: S3RouteConfig): Promise<void> {
