...
export function parseDeleteObjects(xml: string): { keys: string[]; quiet: boolean } {
  ...
  if (keys.length === 0) {
    throw new S3Error("MalformedXML", "The XML you provided was not well-formed.", 400);
  }

  return { keys, quiet: elementValue(xml, "Quiet")?.toLowerCase() === "true" };
}
...
export function parseCompletedParts(xml: string): CompletedPart[] {
  const parts = [...xml.matchAll(/<Part(?:\s[^>]*)?>([\s\S]*?)<\/Part>/gi)].map((match) => {
    const partXml = match[1] ?? "";
    const partNumber = Number.parseInt(elementValue(partXml, "PartNumber") ?? "", 10);
