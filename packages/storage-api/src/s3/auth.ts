...
function canonicalUri(pathname: string): string {
  try {
    ...
    throw new S3Error("InvalidURI", "Could not parse the specified URI.", 400, pathname);
  }
}

function canonicalQuery(url: URL, presigned: boolean): string {
  return [...url.searchParams.entries()]
    .filter(([name]) => !(presigned && name.toLowerCase() === "x-amz-signature"))
...
function validateTime(fields: SignatureFields, now: Date): void {
  ...
  if (fields.presigned) {
    const expiresAt = requestTime + (fields.expiresSeconds ?? 0) * 1000;
...
function canonicalHeaders(request: Request, signedHeadersValue: string, url: URL): string {
  const signedHeaders = signedHeadersValue.split(";");
  const sorted = [...new Set(signedHeaders)].sort();
  if (signedHeaders.join(";") !== sorted.join(";") || !signedHeaders.includes("host")) {
    throw new S3Error(
      "AuthorizationHeaderMalformed",
  ...
  return signedHeaders
    .map((name) => {
      ...
      if (value === null) {
        throw new S3Error("SignatureDoesNotMatch", `Signed header ${name} is missing.`, 403);
      }
...
export function assertPayloadHash(expected: string | null, actual: string): void {
  if (!expected || expected === "UNSIGNED-PAYLOAD") return;
  ...
  if (!expected.match(/^[0-9a-fA-F]{64}$/) || expected.toLowerCase() !== actual.toLowerCase()) {
    throw new S3Error(
...
export function verifyS3Request(request: Request, config: S3AuthConfig, now = new Date()): void {
  ...
  if (scope.length !== 5 || !accessKeyId || !date || !region) {
    throw new S3Error("AuthorizationHeaderMalformed", "The credential scope is malformed.", 400);
  }
  ...
  if (region !== config.region || service !== "s3" || terminator !== "aws4_request") {
    throw new S3Error(
      "AuthorizationHeaderMalformed",
      `The authorization scope must use region '${config.region}' and service 's3'.`,
      400,
    );
  }
  ...
  const actualBuffer = Buffer.from(fields.signature, "hex");
